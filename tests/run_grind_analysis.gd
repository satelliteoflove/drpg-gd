...
func _run_fresh_start_grind() -> void:
	...
	for member in party.get_members():
		var xp_needed := ExperienceTable.get_xp_to_next_level(member.experience, member.level, member.race, member.character_class)
...
func _run_level_5_to_6_grind() -> void:
	...
	for member in party.get_members():
		member.experience = ExperienceTable.get_required_xp(5, member.race, member.character_class)
...
func _create_level_1_party() -> Party:
	...
	var fighter1 := _create_character("Roland", CharacterEnums.CharacterClass.FIGHTER, 1)
	var fighter2 := _create_character("Gareth", CharacterEnums.CharacterClass.FIGHTER, 1)
	var priest := _create_character("Elena", CharacterEnums.CharacterClass.PRIEST, 1)
	var thief := _create_character("Shade", CharacterEnums.CharacterClass.THIEF, 1)
	var mage := _create_character("Vance", CharacterEnums.CharacterClass.MAGE, 1)
	var bishop := _create_character("Aldric", CharacterEnums.CharacterClass.BISHOP, 1)
...
func _create_character(char_name: String, char_class: CharacterEnums.CharacterClass, _level: int) -> Character:
	var stats := {
...
func _create_trained_level_4_party() -> Party:
	...
	for member in [fighter1, fighter2, priest, thief, mage, bishop]:
		...
		member.level = 4
		member.experience = ExperienceTable.get_required_xp(4, member.race, member.character_class)
...
func _create_fighter_only_party() -> Party:
	...
	for i in range(6):
		var fighter := _create_character("Fighter%d" % (i + 1), CharacterEnums.CharacterClass.FIGHTER, 1)
